 ...
 *
 * Discord verlangt eine Antwort innerhalb von 3 Sekunden, sonst zeigt der Client
 * "hat nicht rechtzeitig reagiert" — deshalb ist die eigentliche Verarbeitung in einen
 * Try/Catch gepackt: ein unerwarteter Fehler (z. B. DB-Ausfall) liefert trotzdem eine
 * gültige, wenn auch generische Antwort statt eines Timeouts/einer Fehlerseite, und wird
 * per error_log() festgehalten (sichtbar im PHP-Error-Log des Servers).
 */
...
try {
    ...
    echo json_encode(ephemeral_response('Es ist ein Fehler aufgetreten. Bitte später erneut versuchen oder im Dashboard antworten.'));
}
